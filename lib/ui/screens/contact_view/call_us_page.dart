...
class _CallUsPageState extends State<CallUsPage>
  ...
  Future<void> _openMaps(PickupLocation loc) async {
    final iframeSrc = _extractIframeSrc(loc.googleMap);
    if (iframeSrc != null && iframeSrc.isNotEmpty) {
      final opened = await ExternalLinkLauncher.openUrl(iframeSrc);
      if (opened) return;
    }

    final googleMap = _decodeHtmlEntities(loc.googleMap.trim());
    if (googleMap.isNotEmpty &&
        googleMap.toLowerCase().contains('google.com/maps') &&
        !_isIframeHtml(googleMap)) {
      final opened = await ExternalLinkLauncher.openUrl(googleMap);
      if (opened) return;
    }
