...
class _PaymentPageState extends State<PaymentPage> {
  ...
  void _setupInitialUI() {
    ...
    if (args != null && args['summary_grand_total'] != null) {
      setState(() {
        ...
        _specialDiscount = _toDouble(args['summary_special_discount']) ?? 0.0;
        if ((_specialDiscount ?? 0) <= 0) {
  ...
  Future<void> _initializePayment() async {
    try {
      ...
      setState(() {
        ...
        // Keep shipping API values; only fallback to order when not already set
        if ((_specialDiscount ?? 0) <= 0) {
          final dynamic rawDisc =
              _orderData?['special_discount'] ?? order?['special_discount'];
          final parsedSpecialDiscount = _toDouble(rawDisc) ?? 0.0;
          if (parsedSpecialDiscount > 0) {
            _specialDiscount = parsedSpecialDiscount;
            final description =
                (_orderData?['special_discount_description'] ??
                        order?['special_discount_description'])
                    ?.toString()
                    .trim();
            _specialDiscountDescription =
                (description != null && description.isNotEmpty)
        ...
        if (!_isFromMyOrdersPayment &&
          ...
          _discountAmount =
              ...
              _discountAmount;
          final argSpecialDiscount = _toDouble(
            widget.arguments?['summary_special_discount'],
          );
          if (argSpecialDiscount != null) {
            _specialDiscount = argSpecialDiscount;
          }
          if ((_specialDiscount ?? 0) <= 0) {
            _specialDiscount = 0.0;
            _specialDiscountDescription = null;
            ...
            final description = widget
                .arguments?['summary_special_discount_description']
                ?.toString()
                .trim();
            if (description != null && description.isNotEmpty) {
              _specialDiscountDescription = description;
            }
          }
          _totalCostExclGst =
  ...
  */
  ...
  Future<Map<String, dynamic>> _buildCouponApiPayload({
    required Map<String, dynamic> oldCart,
    required String code,
  }) async {
    final checkoutData = await StorageService.getCheckoutData();
    final shippingMethod =
        checkoutData?['shipping_method'] as String? ?? 'Ship to Address';
    final shippingTypeMethod = shippingMethod == 'Pickup' ? 'pickup' : 'shipto';
    final postcode =
        (_postcode ?? checkoutData?['post_code']?.toString().trim() ?? '');
  ...
  Future<void> _applyPromoCode() async {
    ...
    try {
      ...
      final response = await _cartService.applyCoupon(
        await _buildCouponApiPayload(oldCart: cartForCoupon, code: code),
      );
  ...
  void _resetAvailableCouponsCache() {
    _hasAttemptedCouponFetch = false;
    _availableCoupons = [];
  }
  ...
  Future<void> _fetchAvailableCoupons({VoidCallback? onUpdate}) async {
    ...
    try {
      ...
      final totalPayableAmount = _amount ?? 0.0;
  ...
  Widget _buildSpecialDiscountDescriptionBox() {
    return Container(
      width: double.infinity,
